# -*- coding: utf-8 -*-
"""
Created on Sat Sep 12 18:20:32 2015
...
save='yes' #yes or no
datapath='E:/Work/Rotation/Some_Tracks/MEI/'
filename='MEIDS'
...
#######################################################
from ctvariables import (ncells,size,simtime,runduration,runvelocity,nominal_d,nominal_v,
min_d,min_v,max_d,max_v,mtau,mscale_d,mscale_v,attractant_source,attractant_concentration,
gtau,sensor_max)

strategy='D'       #Stategy to use (V,D or VD: vary velocity, duration or both)
...
for i in np.arange(0,ncells,1):
    ...
    while t<simtime+init_runduration:    
        ...
        gradient_memory=ct.memory_EI(concentration,prev_concentration,prev_memory,elapsed_time,mtau)

        #Calculate next run velocity, duration and displacement
        if strategy=='D':
            runduration=ct.update_runduration_with_memory(nominal_d,init_runduration,gradient_memory,
                                                          mscale_d)
            runduration=ct.laplace_estimate(runduration,min_d,max_d)
            runvelocity=ct.gaussian_estimate(init_runvelocity,min_v,max_v)
            
        if strategy=='V':        
            runvelocity=ct.update_runvelocity_with_memory(nominal_v,init_runvelocity,gradient_memory,
                                                          mscale_v)
